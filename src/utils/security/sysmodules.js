...
function assertMenu(menu) {
  if ((!menu.path && !menu.routePath) || menu.routeNamespace == "/") {
    throw Error(
      "Invalid menu routePath or routeNamespace, see e.g: menu.routePath=/iam/user/list, menu.routeNamespace=/list  =>  menu.permission: " +
        menu.permission
    )
  }
}
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.path === "/login") {
    ...
    try {
      ...
      let flatOneChildRoutes = transform2OneChildrenRoutes(
        cloneGenerateRoutes.slice()
      )
      ...
      ;(function setAsyncRouter(list) {
        list.forEach((item, index) => {
          if (item.type == "1") {
            ...
            throw Error(
              "Failure to dynamic render menu, because menu type is required or unsupported! menu.type=" +
                item.type
            )
          }
          // 处理标签栏标题显示
      ...
      store.commit("update_routList", {
        routList: rootDeepChildRoutes,
      })
