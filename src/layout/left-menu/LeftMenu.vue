<template>
  <div
    class="left"
    :style="{ height: 'calc(100% - 50px)', width: $store.state.leftmenu.width }"
    id="admin-left"
  >
    <div id="left-menu" class="dynamic-menus">
      ...
      >
        <i
          :class="[
            { 'el-icon-arrow-left': $store.state.leftmenu.menu_flag },
            { 'el-icon-arrow-right': !$store.state.leftmenu.menu_flag },
          ]"
        ></i>
      </div>
    </div>
    <transition name="sidebar-fade">
      <div class="sidebar-lightbox" v-show="lightBoxVisible">
        <div class="sidebar-lightbox-list">
          <div
            class="sidebar-lightbox-header"
            @mouseenter="beforeOpenMaskLayer"
            @mouseleave="resetMaskLayer"
          ...
          >
            <el-menu-item
              ...
              v-if="
                !item.hidden &&
                (($store.state.user.userinfo.access_status === 1 &&
                  $store.state.user.userinfo.web_routers[item.path]) ||
                  $store.state.user.userinfo.access_status !== 1)
              "
            >
              <svg
                class="top-menu-iconfont"
                aria-hidden="true"
    ...
    <div
      ...
      @click="parentLevelMenuClick"
    ></div>
    <transition name="mask-fade">
      <div class="menu-list-mask" v-show="maskVisible">
        <div
          class="list-mask-search-bar"
          :class="isKeyWordFocus ? 'active' : ''"
        >
          <el-input
            class="list-mask-search-input"
            ref="maskSearchInput"
            v-model="keyword"
            :placeholder="$t('message.common.searchInput')"
            prefix-icon="el-icon-search"
            @keyup.native="handleKeyWordSearch"
            @focus="isKeyWordFocus = true"
        ...
        <div class="mask-list">
          <div
            ...
            v-for="(classifyData, classifyKey) of menusOfClassify"
          >
            <p class="mask-list-item-title">
              {{ getClassifyName(classifyData) }}
            </p>
            <ul class="mask-list-item-ul">
              <li v-for="item of classifyData.items">
                <span
                  class="mask-list-item-link"
                  @click="handleRouteLinkClick(item.routePath)"
                >
                  {{ getMenuName(item) }}</span
                >
              </li>
            </ul>
          </div>
        ...
        <i
          class="mask-close-btn el-icon-close"
          @click="parentLevelMenuClick"
        ></i>
      </div>
    </transition>
