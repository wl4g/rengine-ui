<template>
  <div class="select-tree-dropdown" v-clickoutside="handleClose">
    ...
    <div class="select-tree-dropdown-wrap" :style="{minWidth: minWidth, top: inputHeight}">
      <div class="search-inp" v-if="filterable">
        <el-input class="tree-search-inp" size="small" placeholder="输入关键字进行过滤" v-model="filterText" @input="runFilter" suffix-icon="el-icon-search">
        </el-input>
      </div>

      <el-tree class="select-tree-dropdown-tree" ref="tree" :data="treeData" :filter-node-method="filterNode" show-checkbox :expand-on-click-node="expandOnClickNode" :check-strictly="checkStrictly" :check-on-click-node="checkOnClickNode" :default-expand-all="defaultExpandAll" @check-change="handleCheckChange" @node-click="handleNodeClick" :node-key="nodeKey" :props="treeProps">
      </el-tree>
    </div>
