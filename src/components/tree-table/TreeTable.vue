<template>
  <el-table :data="formatData" v-bind="$attrs" :row-key="rowKey">
    ...
    <el-table-column v-for="(column, index) in columns" v-else :key="column.value" :label="column.text" :width="column.width" :show-overflow-tooltip="true">
      <template slot-scope="scope">
    ...
    <el-table-column v-if="btn_info.all!==false" :label="btn_info.label || '操作'" :width="btn_info.width || 160" :context="_self">
      <template slot-scope='scope'>
        <span v-if="btn_info.default!==false">
          <a class="table_a" @click='onBtnEvent({type:"Add",data:scope.row,dataIndex:scope.$index})'>{{btn_info.add_text || ''}}</a>|
          <a class="table_a" @click='onBtnEvent({type:"Update",data:scope.row,dataIndex:scope.$index,list:allData})'>{{btn_info.update_text || ''}}</a>|
          <a class="table_a" @click='onBtnEvent({type:"Delete",data:scope.row,dataIndex:scope.$index})'>{{btn_info.delete_text || ''}}</a>
        </span>
        <a v-if='btn_info.list && (!btn_info.list_position || btn_info.list_position==="after") && ((!btn.condition || typeof btn.condition!=="function") || (typeof btn.condition==="function" && btn.condition({list:list,data:scope.row,dataIndex:scope.$index,btnIndex:index,btn:btn})===true))' v-for='(btn,index) in btn_info.list' :key='index' :type="btn.type || 'info'" size="mini" @click='onCustomBtnEvent({list:allData,data:scope.row,dataIndex:scope.$index,btnIndex:index,btn:btn})'>
          {{btn.text}}
        </a>
      </template>
    </el-table-column>
